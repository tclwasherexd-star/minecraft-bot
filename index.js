...
function createBot() {
  ...
  bot.on('spawn', () => {
    ...
    if (useAuthPlugin) {
      setTimeout(() => { bot.chat(`/register ${accountPassword} ${accountPassword}`); bot.chat(`/login ${accountPassword}`); }, 2000);
    }
    
    // Smooth Riding / Attachment Clock Loop (Runs 20 times a second)
    setInterval(() => {
      if (attachTarget) {
    ...
    // Anti-AFK Routine
    setInterval(() => {
      if (!bot.pathfinder.isMoving() && !attachTarget) { bot.setControlState('jump', true); setTimeout(() => bot.setControlState('jump', false), 500); }
    }, 30000);
  });
    ...
    if (command === '!attachmob') {
      ...
      for (const id in bot.entities) {
        ...
        if (e.type === 'mob' || e.type === 'animal' || e.type === 'monster') {
          const d = bot.entity.position.distanceTo(e.position); if (d  bot.whisper(username, e.message)); return; }
