const mineflayer=require('mineflayer'),express=require('express'),{pathfinder,Movements,goals}=require('mineflayer-pathfinder'),app=express();
const config={host:'nbtplace.play.hosting',port:25565,username:'CloudAFK_Bot',version:'1.20.1',auth:'offline'};
const myUsername='tcl',useAuthPlugin=true,accountPassword='YourBotPassword123';
...
function createBot(){
  ...
  bot.on('spawn',()=>{
    console.log(`${bot.username} connected!`);const mcData=require('minecraft-data')(bot.version);
    defaultMove=new Movements(bot,mcData);defaultMove.canDig=false;defaultMove.allow1by1towers=false;bot.pathfinder.setMovements(defaultMove);
    if(useAuthPlugin){setTimeout(()=>{bot.chat(`/register ${accountPassword} ${accountPassword}`);bot.chat(`/login ${accountPassword}`);},2000);}
    setInterval(()=>{if(attachTarget){let e=(attachType==='player')?bot.players[attachTarget]?.entity:bot.entities[attachTarget];if(e){bot.entity.position=e.position.offset(0,e.height,0);}else{attachTarget=null;attachType=null;}}},50);
    setInterval(()=>{if(!bot.pathfinder.isMoving()&&!attachTarget){bot.setControlState('jump',true);setTimeout(()=>bot.setControlState('jump',false),500);}},30000);
  });
  bot.on('whisper',(username,message)=>{
    console.log(`[MSG] ${username}: ${message}`);if(username.toLowerCase()!==myUsername.toLowerCase()){bot.whisper(username,"Access denied.");return;}
    ...
    if(command==='!weather'){bot.whisper(username,bot.isRaining?"Raining/Snowing Mouse":"Clear");return;}
    ...
    if(command==='!attachplayer'){const pTarget=args[1];if(!pTarget||pTarget==='stop'){attachTarget=null;attachType=null;bot.whisper(username,"Detached.");return;}if(!bot.players[pTarget])return bot.whisper(username,"Offline.");attachTarget=pTarget;attachType='player';bot.pathfinder.setGoal(null);bot.whisper(username,`Attached to ${pTarget}`);return;}
    if(command==='!attachmob'){if(args[1]==='stop'){attachTarget=null;attachType=null;bot.whisper(username,"Detached.");return;}let closest=null,min=999;for(const id in bot.entities){const e=bot.entities[id];if(e.type==='mob'||e.type==='animal'||e.type==='monster'){const d=bot.entity.position.distanceTo(e.position);if(dbot.whisper(username,e.message));return;}
    ...
    if(command==='!equip'){const itemName=args.slice(1).join('_').toLowerCase(),item=bot.inventory.items().find(i=>i.name.includes(itemName));if(!item)return bot.whisper(username,"Item not found.");bot.equip(item,'hand').then(()=>bot.whisper(username,`Equipped ${item.name}`)).catch(e=>bot.whisper(username,e.message));return;}
  ...
  bot.on('end',()=>setTimeout(createBot,15000));bot.on('error',(err)=>console.log('Error:',err));
}
createBot();app.get('/',(req,res)=>res.send('Live!'));app.listen(process.env.PORT||3000);
